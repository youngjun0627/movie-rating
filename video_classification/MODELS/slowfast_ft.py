...
def get_slowfast(device):
    ...
    cfg_file = os.path.join(root,'configs/Kinetics/Custom_config.yaml')
    cfg.merge_from_file(cfg_file)
    slowfast_model = build_model(cfg, gpu_id=device)
    cfg.TRAIN.CHECKPOINT_TYPE = 'caffe2'
    cfg.TRAIN.CHECKPOINT_FILE_PATH = os.path.join(root,'models/SLOWFAST_8x8_R50.pkl')
    #slowfast_model.load_state_dict(torch.load(cfg.TRAIN.CHECKPOINT_FILE_PATH), strict=False)
