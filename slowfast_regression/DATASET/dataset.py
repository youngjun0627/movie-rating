...
class VideoDataset(Dataset):
    def __init__(self, directory,size=224, mode = 'train', frame_sample_rate=1, cut_time=1, play_time=680,transform=None,sub_classnum=6):
        folder = directory
        self.size = size
        ...
        with open(csv_file, 'r', encoding='utf-8-sig') as f:
            ...
            for idx, data in enumerate(rdr):
                ...
                if frame_length<self.play_time+2*self.cut_time or frame_length>self.play_time*1.5:
                    continue
                filename = data[1]
                '''
                labels = []      
    ...
    def __getitem__(self,index):
        ...
        if frame_length<2*self.cut_time+self.play_time:
            start = self.cut_time
        else:
            start = random.randrange(self.cut_time, frame_length-self.cut_time-self.play_time+1)
...
if __name__=='__main__':
    ...
    transform = create_train_transfrom(True,True,True,True)
    path = '/mnt/data/guest0/uchan/DATA'
    a = VideoDataset(path, transform = transform)
