...
class SlowFast(nn.Module):
    def __init__(self, block = Bottleneck, layers=[3,5,11,7], class_num=4, label_num = 5, dropout=0.5, mode='multi', vocab_size = 0):
        ...
        self.classifier1 = nn.ModuleList([nn.Linear(self.fast_inplanes+2048+self.audio_size, class_num) for _ in range(label_num)])
        ...
        # genre fc, age fc #
        self.genrefc = nn.Linear(self.fast_inplanes + 2048 + self.audio_size, 9)
        self.agefc = nn.Linear(self.fast_inplanes + 2048 +  self.audio_size, 4)
