...
def train(model, train_dataloader, epoch, criterion1, criterion2, criterion3, optimizer, writer, device, mode = 'single', label_num=1):
    ...
    age_preds = []
    age_targets = []
    for step, (video, text, labels, age, genre) in enumerate(train_dataloader):
        ...
        outputs, genre, age = model(video,text)
...
def val(model, val_dataloader, epoch, criterion1, criterion2, criterion3, optimizer, writer, device, mode = 'single', label_num=1):
    ...
    running_loss = 0
    with torch.no_grad():
        for step, (video,text, labels, age, genre) in enumerate(val_dataloader):
            data_time.update(time.time()-end)
            video = video.to(device)
            text = text.to(device)
            labels = labels.long().to(device)
