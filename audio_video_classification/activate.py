...
def train(model, train_dataloader, epoch, criterion1, criterion2, criterion3, optimizer, writer, device, mode = 'single', label_num=1):
    ...
    age_preds = []
    age_targets = []
    for step, (video, audio, labels, age, genre) in enumerate(train_dataloader):
        ...
        outputs, genre, age = model(video,audio)
        ...
        if (step+1)==len(train_dataloader):
            ...
            print(print_string)
            if mode=='single':
                print_string = 'Loss : {loss:.5f} \t F1_score : {metric:.5f}'.format(loss=running_loss/len(train_dataloader), metric = f1_score(np.array(targets), np.array(preds), average='weighted'))
                print(print_string)
                targets.clear()
                preds.clear()
            elif mode=='multi':
                ...
                for idx, (pred, target) in enumerate(zip(preds, targets)):
                    ...
                    score = f1_score(np.array(target, nparray(pred), average='weighted')
...
def val(model, val_dataloader, epoch, criterion1, criterion2, criterion3, optimizer, writer, device, mode = 'single', label_num=1):
    ...
    running_loss = 0
    with torch.no_grad():
        for step, (video, audio, labels, age, genre) in enumerate(val_dataloader):
            data_time.update(time.time()-end)
            video = video.to(device)
            audio = audio.to(device)
            labels = labels.long().to(device)
