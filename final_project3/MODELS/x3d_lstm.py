...
class Bottleneck(nn.Module):
    def __init__(self, in_planes, planes, stride=1, downsample=None, index=0, base_bn_splits=8):
        ...
        #self.bn1 = SubBatchNorm3d(num_splits=self.base_bn_splits, num_features=planes[0], affine=True) 
        self.bn1 = nn.BatchNorm3d(planes[0])
        self.conv2 = conv3x3x3(planes[0], planes[0], stride)
        #self.bn2 = SubBatchNorm3d(num_splits=self.base_bn_splits, num_features=planes[0], affine=True) 
        self.bn2 = nn.BatchNorm3d(planes[0])
        self.conv3 = conv1x1x1(planes[0], planes[1])
        #self.bn3 = SubBatchNorm3d(num_splits=self.base_bn_splits, num_features=planes[1], affine=True) 
...
class ResNet(nn.Module):
    ...
    def forward(self, x, text, offset, audio):
        ...
        for i in range(0, x.shape[2], 64):
            ...
            _video = self.conv1_t(_video)
            _video = self.bn1(_video)
            _video = self.relu(_video)
    
            _video = self.layer1(_video)
            _video = self.layer2(_video)
            _video = self.layer3(_video)
            _video = self.layer4(_video)
        
            _video = self.conv5(_video)
            _video = self.bn5(_video)
            _video = self.leakyrelu(_video)

            _video = self.avgpool(_video)

            _video = self.fc1(_video)
            _video = self.relu(_video)
            _video = _video.view(-1,_video.size(1))
            video.append(_video)
        ...
        #print(video.shape)
        #video = x.squeeze(4).squeeze(3).squeeze(2) # B C
        #print(video.shape)
        #print(audio.shape)
        #print(text.shape)
...
if __name__=='__main__':
    ...
    for parameter in list(model.parameters())[-16:]:
        print(parameter.shape)
    #for parameter in list(model.fcs.parameters()):
        #print(parameter.shape)
