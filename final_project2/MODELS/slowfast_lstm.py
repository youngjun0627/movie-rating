...
class SlowFast(nn.Module):
    def __init__(self, block = Bottleneck, layers=[3,5,11,7], class_num=4, label_num = 5, dropout=0.7, mode='multi', vocab_size = 0):
        ...
        self.classifier1 = nn.ModuleList([nn.Linear(128*32+self.embed_dim+self.audio_size, class_num) for _ in range(label_num)])
        ...
        # genre fc, age fc #
        self.genrefc = nn.Linear(128*32 + self.embed_dim + self.audio_size, 9)
        ...
        self.lstm = nn.LSTM(self.fast_inplanes + 2048, hidden_size = 128, num_layers = 2, batch_first=False, bidirectional=True, dropout=0.2) 
