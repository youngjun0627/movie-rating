...
from MODELS import slowfastnet, x3d, multi_slowfastnet, multi_x3d, efficientnet, multi_x3d_plot_multitask, multi_x3d_plot, multi_x3d_plot_multitask_audio, multi_slowfastnet_plot_multitask_audio, multi_slowfast_notplot_multitask_audio, multi_x3d_notplot_multitask_audio
...
def main():
    ...
    train_transform = create_train_transform(True,True,True,True, size=params['size'], bright=True)
    ...
    ### multi ###
        ...
        if params['model']=='slowfast':
            ...
            if params['use_plot']:
                ...
                train_dataset.generate_text_pipeline(vocab,tokenizer)
                val_dataset.generate_text_pipeline(vocab, tokenizer) 
                model = multi_x3d_notplot_multitask_audio.generate_model('S', n_classes = params['num_classes'], label_num = params['label_num'], vocab_size = len(vocab))
            ...
            if params['use_plot']:
                ...
                train_dataset.generate_text_pipeline(vocab,tokenizer)
                val_dataset.generate_text_pipeline(vocab, tokenizer) 
                model = multi_slowfast_notplot_multitask_audio.resnet50(class_num=params['num_classes'], label_num = params['label_num'], mode = params['mode'], vocab_size = len(vocab))
                init_weights(model)
    ...
    #scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=10, T_mult=1, eta_min = 0.0001)
