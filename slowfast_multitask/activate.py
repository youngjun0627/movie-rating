...
def train(model, train_dataloader, epoch, criterion1, criterion2, criterion3, optimizer, writer, device, mode = 'single', label_num=1, display=1):
    ...
    age_preds = []
    age_targets = []
    for step, (inputs,text,offset, labels, age, genre) in enumerate(train_dataloader):
        ...
        outputs, genre, age = model(inputs,text,offset)
...
def val(model, val_dataloader, epoch, criterion1, criterion2, criterion3, optimizer, writer, device, mode = 'single', label_num=1):
    ...
    running_loss = 0
    with torch.no_grad():
        for step, (inputs,text,offset, labels, age, genre) in enumerate(val_dataloader):
            data_time.update(time.time()-end)
            inputs=inputs.to(device)
            text = text.to(device)
            offset = offset.to(device)
