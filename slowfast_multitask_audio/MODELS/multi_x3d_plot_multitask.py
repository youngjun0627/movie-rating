...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.fcs = nn.ModuleList([nn.Linear(2048+embed_dim, n_classes) for _ in range(label_num)])
        ...
        self.genrefc = nn.Linear(2048+embed_dim, 9)
        self.agefc = nn.Linear(2048+embed_dim, 4)
    ...
    def forward(self, x, text, offset):
        x = self.conv1_s(x)
        x = self.conv1_t(x)
        x = self.bn1(x)
        x = self.relu(x)
