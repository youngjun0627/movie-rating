...
def train(model, train_dataloader, epoch, criterion, optimizer, writer, device, mode = 'single', label_num=1, display=1):
    ...
    for step, (inputs,text,offset, labels) in enumerate(train_dataloader):
        ...
        outputs = model(inputs,text,offset).to(device)
        print(outputs.shape)
        print(labels.shape)
...
def val(model, val_dataloader, epoch, criterion, optimizer, writer, device, mode = 'single', label_num=1):
    ...
    running_loss = 0
    with torch.no_grad():
        for step, (inputs,text,offset, labels) in enumerate(val_dataloader):
            data_time.update(time.time()-end)
            inputs=inputs.to(device)
            text = text.to(device)
            offset = offset.to(device)
